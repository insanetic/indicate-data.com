/**
 * German text verbatim from indicate-data.io (DE original), which is the legally
 * binding version. The live site publishes no English privacy policy, so \`en\` is
 * our own translation for convenience only.
 */
const de = `Wir verpflichten uns, die Privatsphäre unserer Besucher und Mitglieder zu schützen. Diese Datenschutzerklärung erläutert, welche persönlichen und anderen Daten von Ihnen durch Indicate Data auf seiner Website (www.indicate-data.io) und unseren Webanwendungen erfasst werden, wie die Informationen verwendet werden und wie Sie die Erfassung, Nutzung, Korrektur und/oder Löschung Ihrer Informationen steuern können. Wir werden Ihre Informationen nicht an Dritte weitergeben oder mit diesen teilen, außer wie in dieser Datenschutzerklärung beschrieben.

Die "Firma", "Indicate Data", "Indicate", "uns", "wir" oder "unser" bezieht sich auf die Indicate Data GmbH., den Eigentümer und Betreiber der Website und Webanwendungen, ein in Deutschland eingetragenes Unternehmen.

Diese Datenschutzerklärung ist Bestandteil unserer Nutzungsbedingungen. MIT DER NUTZUNG UNSERER WEBSITE / WEBANWENDUNGEN STIMMEN SIE DEN BEDINGUNGEN DIESER DATENSCHUTZERKLÄRUNG ZU.

Bitte lesen Sie diese Datenschutzerklärung sorgfältig durch, um unsere Datenschutzpraktiken zu verstehen. Wenn Sie diesen Praktiken nicht zustimmen, besteht Ihre Abhilfemaßnahme darin, die Website oder Webanwendungen nicht zu nutzen.

## Welche Informationen wir sammeln

Informationen, die Sie uns zur Verfügung stellen - Sie geben uns Informationen über sich selbst, wie Ihren Namen und Ihre E-Mail-Adresse, wenn Sie sich bei uns registrieren. Wenn Sie per E-Mail, Telefon oder über die Website mit uns korrespondieren, können wir den Inhalt Ihrer Nachrichten, Ihre E-Mail-Adresse oder Telefonnummer und unsere Antworten speichern.

Registrierung auf unserer Website - Personen, die auf unsere Webanwendungen zugreifen, werden gebeten, sich über unsere Website zu registrieren (Anmeldung für ein kostenloses/bezahltes Konto). Bei der Registrierung werden Sie aufgefordert, einige persönliche Daten anzugeben: Ihren Namen, die Unternehmenszugehörigkeit und die Unternehmens-E-Mail-Adresse. Auch ein einzigartiges Passwort für den Zugriff auf Ihr Konto ist erforderlich. (Bitte teilen Sie Ihr Passwort niemandem mit.)

Wenn Sie sich registrieren und ein Konto einrichten und beim Zugriff auf unsere Webanwendungen werden Sie gebeten, Ihre zugehörigen Profileinstellungen zu bestätigen, einschließlich Informationen darüber, wie Sie kontaktiert werden möchten.

Sie können sich auch über Google registrieren und ein Konto erstellen. Persönliche Daten, die Sie Google zur Verfügung gestellt haben, werden uns zur Einrichtung Ihres Kontos zur Verfügung gestellt. Wenn Sie sich über Google für ein Konto bei uns anmelden, stimmen Sie dieser Datenschutzerklärung und unseren Nutzungsbedingungen zu.

Wir sammeln keine "sensiblen" oder "besonderen" Kategorien personenbezogener Daten.

Nutzerinhalte - Es kann öffentliche Bereiche auf der Website oder in den Webanwendungen geben, in denen Benutzer Nachrichten veröffentlichen, an öffentlichen Schwarzen Brettern teilnehmen können. Die Regeln für die Veröffentlichung von Nutzerinhalten werden durch unsere Nutzungsbedingungen geregelt.

Informationen, die Sie automatisch bereitstellen - Wenn Sie auf die Website zugreifen, erfassen wir automatisch bestimmte persönliche und andere Informationen, beispielsweise über das Gerät, das Sie verwenden, und wie Sie mit der Website und den Webanwendungen interagieren. Erfasste Informationen können speziell umfassen: (1) die verwendeten Browsertypen und -versionen, (2) das vom Gerät verwendete Betriebssystem, (3) die Website, von der aus Sie auf unsere Website zugreifen (sogenannte Referrer), (4) die Unterwebsites, (5) das Datum und die Uhrzeit (sogenannter Zeitstempel) des Zugriffs auf die Internetseite und (6) die Internetprotokolladresse (IP-Adresse), die Ihrem Gerät zugeordnet ist. Wir speichern diese Informationen in Serverprotokolldateien. Wir erfassen auch automatisch Informationen über Ihre Nutzung der Website und Webanwendungen, wie Webanfragen, Anzahl der Klicks, Domainnamen, angesehene Seiten und die Zeit, die auf bestimmten Seiten verbracht wurde.

Diese Informationen werden benötigt, um (1) den Inhalt unserer Website und Webanwendungen korrekt zu liefern, (2) den Inhalt unserer Website und Webanwendungen zu optimieren, (3) die langfristige Lebensfähigkeit unserer Informationstechnologiesysteme und der Website-Technologie zu gewährleisten und (4) den Strafverfolgungsbehörden die für die strafrechtliche Verfolgung im Falle eines Cyberangriffs notwendigen Informationen zur Verfügung zu stellen.

Darüber hinaus werden beim Navigieren durch unsere Website oder Webanwendungen Tracking-Informationen gesammelt. Um Ihren Bedürfnissen besser gerecht zu werden, verwenden wir "Cookies", um Benutzerinformationen zu speichern und manchmal zu verfolgen. Ein Cookie ist eine kleine Datenmenge, die von einem Webserver an Ihren Browser gesendet und auf der Festplatte Ihres Geräts gespeichert wird. Cookies können deaktiviert oder durch Einstellungen in Ihrem Browser gesteuert werden. Sie können Cookies auch löschen oder ablehnen. Weitere Informationen finden Sie in unserer Cookie-Richtlinie.

Darüber hinaus werden Protokolldateiinformationen automatisch von Ihrem Browser jedes Mal gemeldet, wenn Sie eine Webseite aufrufen. Wenn Sie sich bei unserer Website oder Webanwendungen registrieren oder diese ansehen, zeichnen unsere Server automatisch bestimmte Informationen auf, die Ihr Browser jedes Mal sendet, wenn Sie eine Website besuchen.

Indicate Data analysiert auch anonym gesammelte Daten und Statistiken, um Ihr Erlebnis zu verbessern und ein optimales Schutzniveau für die von uns verarbeiteten personenbezogenen Daten sicherzustellen. Die anonymen Daten der Serverprotokolldateien werden getrennt von allen personenbezogenen Daten, die von Benutzern bereitgestellt werden, gespeichert.

Unternehmensinformationen - Sie können weitere Informationen über sich und Ihr Unternehmen bereitstellen, wie Ihre Rolle, Branche, Unternehmensgröße, Jahresumsatz usw. Solche Informationen werden sicher gespeichert und können für Benchmarking und andere Zielsetzungen verwendet werden. Indicate Data kann solche Informationen mit Hilfe von Drittanbietern als Unterauftragnehmer erfassen, um ein besseres Erlebnis und Profiling zu gewährleisten. Sie können diese Informationen jederzeit aktualisieren oder korrigieren, indem Sie sich in Ihr Konto einloggen.

Diese personenbezogenen Daten werden nicht an Dritte weitergegeben. Sie können sich jederzeit von unserem Online-Newsletter abmelden, indem Sie auf den Link "Abmelden" am Ende unserer E-Mails klicken oder uns direkt unter: [compliance@indicate-data.io](mailto:compliance@indicate-data.io) kontaktieren.

## Benchmarks

Indicate Data kann Ihre Daten verwenden, um Benchmarking-Dienste zu berechnen und bereitzustellen. Die für die Teilnahme am Benchmarking-Ökosystem gesammelten Daten werden jederzeit anonymisiert und von jeglichen persönlichen oder sensiblen Informationen getrennt.

Um die Anonymität zu gewährleisten, werden Benchmarks nur berechnet und verfügbar gemacht, wenn Indicate Data mindestens 10 verschiedene Datenquellen innerhalb der ausgewählten Kohorte hat.

Sie können jederzeit ablehnen, an Benchmarks teilzunehmen, indem Sie sich in Ihr Konto einloggen und sich abmelden.

## Wie wir Ihre Informationen verwenden

Im Allgemeinen verwenden wir die gesammelten personenbezogenen Daten und anderen Informationen, um Ihnen die bestmögliche Erfahrung bei der Nutzung der Indicate Data-Website und -Webanwendungen zu bieten. Diese Informationen helfen uns auch zu verstehen, wer unsere Produkte nutzt und wie wir unsere Dienstleistungen verbessern können, um unseren Helpdesk-Service zu bieten, Sie aus Kundenservice- und Abrechnungsgründen zu kontaktieren und die Bereitstellung von Indicate Data-Marketinginformationen über unsere Produkte und Dienstleistungen zu erleichtern.

Wir verwenden die von Ihnen übermittelten personenbezogenen Daten, um die Funktionen und Funktionalitäten unserer Produkte und Dienstleistungen für Sie zu betreiben, aufrechtzuerhalten und bereitzustellen. Jede personenbezogene Daten oder Nutzerinhalte, die Sie durch das Veröffentlichen auf Indicate Data freiwillig offenlegen, können für die anderen Benutzer in Ihrem Konto verfügbar werden. Wenn Sie Nutzerinhalte entfernen, können Kopien in zwischengespeicherten und archivierten Seiten sichtbar bleiben oder wenn andere Benutzer Ihre Nutzerinhalte kopiert oder gespeichert haben.

Wir werden Ihre registrierte E-Mail-Adresse verwenden, um Ihnen Indicate Data-bezogene Hinweise zu Produkten oder Dienstleistungen zu senden, die Sie angefordert haben, zum Status Ihres Kontos oder zu anderen administrativen oder technischen Informationen, einschließlich der gesetzlich erforderlichen Hinweise. Wir können Ihre E-Mail-Adresse auch verwenden, um Ihnen andere Nachrichten zu senden, wie z. B. Newsletter, Änderungen an Funktionen innerhalb von Indicate Data oder Sonderangebote. Wenn Sie solche E-Mail-Nachrichten nicht erhalten möchten, können Sie sich jederzeit in Ihren Profileinstellungen abmelden oder auf den Link "Abmelden" am Ende unserer E-Mails klicken. Sie können sich auch abmelden, um unsere Marketingnachrichten zu erhalten, indem Sie uns unter [compliance@indicate-data.io](mailto:compliance@indicate-data.io) kontaktieren. Wir geben Ihre E-Mail oder andere personenbezogenen Daten nicht an Dritte für deren Marketingaktivitäten weiter. Wir setzen auch kein zielgerichtetes Werben um, weder auf unserer Website noch auf Websites Dritter.

Wir verwenden Cookies, klare GIFs und Informationen aus Serverprotokolldateien, um Informationen zu speichern, damit Sie diese während Ihres Besuchs oder beim nächsten Besuch der Website nicht erneut eingeben müssen; um individuelle, personalisierte Inhalte und Informationen bereitzustellen; die Wirksamkeit unseres Dienstes zu überwachen; aggregierte Kennzahlen wie die Gesamtzahl der Besucher und den Verkehr zu überwachen; technische Probleme zu diagnostizieren oder zu beheben, die von unseren Benutzern oder Ingenieuren gemeldet wurden und die mit den IP-Adressen eines bestimmten Webunternehmens oder ISPs in Verbindung stehen; und Ihnen zu helfen, nach der Anmeldung effizient auf Ihre Informationen zuzugreifen. Weitere Informationen über unsere Verwendung von Cookies finden Sie in unserer Cookie-Richtlinie.

## Wie wir Ihre Informationen teilen

Wir können unabhängige Auftragnehmer, Anbieter und Lieferanten (zusammenfassend "externe Auftragnehmer") einsetzen, um bestimmte Dienstleistungen und Produkte im Zusammenhang mit der Indicate Data-Website und unseren Dienstleistungen bereitzustellen, wie Hosting, Kundenservice, Kreditkartenabwicklung und Betrugsprüfung sowie Hosting von Mailinglisten. Im Rahmen der Bereitstellung von Produkten oder Dienstleistungen für uns können diese externen Auftragnehmer Zugang zu über die Indicate Data-Website oder -Webanwendung gesammelten Informationen, einschließlich Ihrer personenbezogenen Daten, haben. Wir verlangen, dass diese externen Auftragnehmer (1) die Privatsphäre Ihrer personenbezogenen Daten gemäß dieser Datenschutzerklärung und den Nutzungsbedingungen schützen und (2) Ihre personenbezogenen Daten nicht für andere Zwecke verwenden oder offenlegen als für die Bereitstellung der Produkte oder Dienstleistungen, für die wir sie beauftragt haben, oder wie gesetzlich vorgeschrieben.

Wenn Sie unsere Website und Webanwendungen nutzen, können Sie mit unseren Geschäftspartnern interagieren. Um Ihnen zusätzliche Unterstützung zu bieten, können Ihr Name und Ihre E-Mail-Adresse mit Partnern geteilt werden.

Wenn wir unser Geschäft weiterentwickeln, können wir Vermögenswerte oder Geschäftsangebote kaufen oder verkaufen. Kunden- und Besucherinformationen sind im Allgemeinen eines der übertragenen Geschäftsvermögen bei diesen Arten von Transaktionen. Wir können solche Informationen auch im Rahmen von Unternehmensveräußerungen, Fusionen oder Auflösungen übertragen.

Indicate Data kann personenbezogene Daten offenlegen, wenn dies gesetzlich erforderlich ist oder durch Vorladung oder wenn wir glauben, dass eine solche Aktion notwendig ist, um das Gesetz einzuhalten, einem uns oder unseren verbundenen Unternehmen zugestellten rechtlichen Verfahren nachzukommen oder Ermittlungen durchzuführen, vorzubeugen oder Maßnahmen in Bezug auf mutmaßliche oder tatsächliche illegale Aktivitäten zu ergreifen; um unsere Nutzungsbedingungen durchzusetzen, Vorkehrungen gegen Haftung zu treffen, uns gegen Ansprüche oder Vorwürfe Dritter zu verteidigen, Strafverfolgungsbehörden zu unterstützen oder die Sicherheit oder Integrität unserer Website zu schützen; und die Rechte, das Eigentum oder die persönliche Sicherheit von Indicate Data, unseren Benutzern oder anderen zu schützen oder auszuüben.

Darüber hinaus können wir statistische Analysen der Kundenutzung durchführen, um Interesse an und Nutzung der verschiedenen Teile der Website zu messen, und können diese Informationen im Rahmen unseres Geschäfts teilen, jedoch nur in anonymisierter und/oder aggregierter Form (Statistiken usw.), und ohne personenbezogene Daten.

Indicate Data vermietet oder verkauft Ihre personenbezogenen Daten nicht an Dritte.

## Von uns eingesetzte Auftragsverarbeiter

Zur Erbringung unserer Dienstleistungen setzen wir sorgfältig ausgewählte Dienstleister als Auftragsverarbeiter im Sinne des Art. 28 DSGVO ein. Mit jedem dieser Dienstleister haben wir einen Vertrag zur Auftragsverarbeitung (AV- Vertrag) geschlossen. Die Auftragsverarbeiter verarbeiten personenbezogene Daten ausschließlich nach unserer Weisung und für die nachstehend genannten Zwecke. Die folgende Übersicht führt die aktuell von uns eingesetzten Auftragsverarbeiter auf:

| Auftragsverarbeiter | Dienst und Zweck der Verarbeitung | Ort der Verarbeitung | Grundlage der Übermittlung |
|---|---|---|---|
| Google Ireland Limited, Gordon House, Barrow Street, Dublin 4, D04 E5W5, Irland | Google Analytics – Reichweitenmessung und Analyse der Nutzung unserer Website | EU/EWR; Übermittlung an die Google LLC, USA | AV-Vertrag nach Art. 28 DSGVO. Die Google LLC ist nach dem EU-U.S. Data Privacy Framework zertifiziert (unter anderem für Google Analytics); Übermittlungen in die USA erfolgen daher auf Grundlage des Angemessenheitsbeschlusses nach Art. 45 DSGVO, ergänzend gelten Standardvertragsklauseln nach Art. 46 DSGVO. |
| Microsoft Ireland Operations Limited, One Microsoft Place, South County Business Park, Leopardstown, Dublin 18, D18 P521, Irland | Microsoft 365 – E-Mail, Office-Anwendungen und Zusammenarbeit, einschließlich der Bearbeitung von Kundenanfragen | EU/EWR; Übermittlung an die Microsoft Corporation, USA | AV-Vertrag nach Art. 28 DSGVO. Die Microsoft Corporation ist nach dem EU-U.S. Data Privacy Framework zertifiziert; Übermittlungen in die USA erfolgen daher auf Grundlage des Angemessenheitsbeschlusses nach Art. 45 DSGVO, ergänzend gelten Standardvertragsklauseln nach Art. 46 DSGVO. |
| Productlane GmbH, Albert-Roßhaupter-Straße 3b, 81369 München, Deutschland | Productlane – Support-Plattform zur Entgegennahme und Bearbeitung von Supportanfragen und Kundenfeedback | EU (Hosting in Frankfurt am Main); einzelne Unterauftragsverarbeiter mit Sitz in den USA | AV-Vertrag nach Art. 28 DSGVO; für Übermittlungen in die USA Standardvertragsklauseln nach Art. 46 DSGVO. |
| Linear Orbit, Inc., 2261 Market Street STE 10632, San Francisco, CA 94114, USA | Linear – Vorgangs- und Aufgabenverwaltung, unter anderem zur Nachverfolgung von Supportanfragen | USA | AV-Vertrag nach Art. 28 DSGVO in Verbindung mit den EU-Standardvertragsklauseln nach Art. 46 DSGVO. Eine Zertifizierung nach dem EU-U.S. Data Privacy Framework liegt nicht vor. |
| Slack Technologies Limited, Salesforce Tower, 60 R801, North Dock, Dublin, Irland | Slack – interne Kommunikation und Abstimmung, unter anderem bei der Bearbeitung von Supportanfragen | EU/EWR; Übermittlung an die Slack Technologies, LLC bzw. die Salesforce, Inc., USA | AV-Vertrag nach Art. 28 DSGVO. Die Salesforce, Inc. ist einschließlich der Slack Technologies, LLC nach dem EU-U.S. Data Privacy Framework zertifiziert; Übermittlungen in die USA erfolgen daher auf Grundlage des Angemessenheitsbeschlusses nach Art. 45 DSGVO, ergänzend gelten Standardvertragsklauseln nach Art. 46 DSGVO. |
| Notion Labs, Inc., 2300 Harrison Street, San Francisco, CA 94110, USA | Notion – interne Dokumentation und Wissensmanagement, unter anderem für Support-Prozesse | USA | AV-Vertrag nach Art. 28 DSGVO in Verbindung mit den EU- und UK-Standardvertragsklauseln nach Art. 46 DSGVO. |

Wir prüfen unsere Auftragsverarbeiter regelmäßig und aktualisieren diese Übersicht, wenn wir Dienstleister hinzufügen oder nicht mehr einsetzen. Wenn Sie Fragen zu einem der genannten Auftragsverarbeiter haben oder eine Kopie der getroffenen Garantien für Drittlandübermittlungen wünschen, wenden Sie sich bitte an [compliance@indicate-data.io](mailto:compliance@indicate-data.io).

## Datenschutzrechte in Kalifornien

California "Shine The Light" - Wie im California Civil Code 1798.83 vorgesehen, haben Sie, wenn Sie ein Einwohner Kaliforniens sind, das Recht, Informationen zu erhalten, die (a) jedes Drittunternehmen benennen, an das wir im vergangenen Jahr Ihre personenbezogenen Daten weitergegeben haben; und (b) eine Beschreibung der Kategorien personenbezogener Daten, die offengelegt wurden. Um solche Informationen anzufordern, senden Sie bitte Ihre Anfrage per E-Mail an [compliance@indicate-data.io](mailto:compliance@indicate-data.io) mit "Anfrage zu Datenschutzrechten in Kalifornien" im Betreff.

California Consumer Privacy Act - Bei der Erbringung seiner Dienstleistungen für seine Kunden kann Indicate Data als "Dienstleister" im Sinne des California Consumer Privacy Act ("CCPA") betrachtet werden. Indicate Data kann auch die Dienste von Dritten nutzen, die ebenfalls als "Dienstleister" im Sinne des CCPA gelten können. Soweit von unseren Kunden gewünscht, unterstützen wir bei der Beantwortung von Anfragen kalifornischer Verbraucher bezüglich ihrer Datenschutzrechte gemäß dem CCPA. Wenn wir eine Anfrage eines kalifornischen Verbrauchers erhalten, zu wissen oder zu löschen, welche personenbezogenen Daten wir im Auftrag unseres Kunden erheben, für den wir Dienstleistungen erbringen, werden wir den Verbraucher anweisen, die Anfrage direkt an unseren Kunden zu richten. Wenn möglich, werden wir dem Verbraucher die Kontaktdaten des Kunden zur Verfügung stellen. Sie können uns unter [compliance@indicate-data.io](mailto:compliance@indicate-data.io) für weitere Informationen zu diesen Rechten kontaktieren.

Wir verkaufen Ihre personenbezogenen Daten nicht an Dritte für irgendeinen Zweck.

## Standort in Deutschland

Indicate Data und unsere Website und Webanwendungen befinden sich in Deutschland und unterliegen deutschem Recht. Wenn Sie sich außerhalb Deutschlands befinden, wenn Sie die Website, die Webanwendung besuchen oder mit uns per Post, E-Mail oder Telefon kommunizieren, beachten Sie bitte, dass Ihre personenbezogenen Daten nach Deutschland übertragen, gespeichert und verarbeitet werden können, wo sich unsere Server befinden. Alle Informationen, die Sie uns zur Verfügung stellen oder die wir durch Ihre Nutzung der Websites oder Webanwendungen erfassen, werden in Deutschland gespeichert, verarbeitet und übertragen. Bitte beachten Sie, dass Deutschland und andere Rechtsordnungen als die, in der Sie sich befinden, möglicherweise nicht denselben Datenschutzstandard bieten wie in Ihrem Land als angemessen erachtet. Beachten Sie auch, dass Ihre personenbezogenen Daten der deutschen Regierung oder deren Behörden im Rahmen eines rechtlichen Verfahrens in Deutschland zur Verfügung stehen können.

## Datenschutzrechte im Europäischen Wirtschaftsraum und im Vereinigten Königreich

### Die Rechtsgrundlage für die Verwendung personenbezogener Daten im EWR und im Vereinigten Königreich

Für Bewohner des Europäischen Wirtschaftsraums (EWR) und des Vereinigten Königreichs (UK) weisen wir darauf hin, dass Ihre personenbezogenen Daten nach Deutschland übertragen und dort verarbeitet werden, das Datenschutzgesetze hat, die von denen in Ihrem Land abweichen und möglicherweise nicht so schützend sind. Deutschland hat keine "Angemessenheitsfeststellung" von der Europäischen Union nach Artikel 45 der Datenschutz-Grundverordnung ("DSGVO") gesucht oder erhalten. Unsere Rechtsgrundlage für die Erhebung und Nutzung Ihrer personenbezogenen Daten besteht darin, dies mit Ihrer Zustimmung zu tun; wo wir die personenbezogenen Daten für die Erfüllung eines Vertrags oder einer angeforderten Dienstleistung benötigen, einschließlich der Bereitstellung unseres Newsletters, oder wo die Erhebung und Nutzung in unserem oder eines anderen berechtigten Interesse liegt und nicht durch Ihre Datenschutzinteressen oder grundlegende Rechte und Freiheiten überwogen wird. In einigen Fällen können wir auch rechtlich verpflichtet sein, die personenbezogenen Daten zu erheben. Wenn wir Ihre personenbezogenen Daten mit Ihrer Zustimmung erhoben haben, können Sie Ihre Zustimmung jederzeit widerrufen.

Unser Beibehalten Ihrer personenbezogenen Daten und aller nachfolgenden Kommunikationen basiert auf unserem berechtigten Interesse, Ihnen neue und potenziell relevante Materialien basierend auf Ihrer Geografie, Rolle oder Firma zur Verfügung zu stellen. Wie immer können Sie sich dafür entscheiden, solche zukünftigen Kommunikationen abzulehnen.

Soweit wir personenbezogene Daten aus dem EWR in ein Gebiet außerhalb des EWR übertragen, das von der Europäischen Kommission nicht als angemessen erachtet wurde (wie Deutschland), werden wir sicherstellen, dass solche personenbezogenen Daten durch geeignete vertragliche Bestimmungen oder andere genehmigte Mechanismen geschützt werden.

### Unsere gesetzliche Verpflichtung, Ihre Informationen offenzulegen

Wir werden personenbezogene Daten eines Benutzers ohne dessen vorherige Zustimmung nur dann offenlegen, wenn wir Grund zu der Annahme haben, dass die Offenlegung dieser Informationen erforderlich ist, um die Identität einer Person oder mehrerer Personen zu ermitteln, zu kontaktieren oder rechtliche Schritte gegen eine Person oder Personen einzuleiten, von denen vermutet wird, dass sie Rechte oder Eigentum von Indicate Data oder anderen, die durch die Aktivitäten des Benutzers geschädigt werden könnten, verletzt haben oder von Personen, die diese Rechte und Eigentum (absichtlich oder anderweitig) verletzen könnten. Wir können personenbezogene Daten auch dann offenlegen, wenn wir guten Grund zu der Annahme haben, dass dies gesetzlich erforderlich ist.

### Rechte für Bewohner des EWR und des Vereinigten Königreichs

Zusätzlich haben Sie, wenn Sie ein Bewohner des EWR oder des Vereinigten Königreichs sind, das Recht:

- Herauszufinden, ob wir Ihre personenbezogenen Daten verwenden, auf Ihre personenbezogenen Daten zuzugreifen und Kopien Ihrer personenbezogenen Daten zu erhalten.
- Jede ausdrückliche Zustimmung, die Sie zur Verarbeitung Ihrer personenbezogenen Daten gegeben haben, jederzeit ohne Strafe zu widerrufen.
- Auf Ihre personenbezogenen Daten zuzugreifen und diese zu korrigieren oder zu ändern, wenn sie ungenau oder unvollständig sind.
- Eine übertragbare Kopie einiger Ihrer personenbezogenen Daten zu erhalten, die auf eine andere Anbieter übertragen werden kann, wenn die personenbezogenen Daten auf der Grundlage Ihrer Zustimmung verarbeitet wurden.
- Wenn Sie glauben, dass Ihre personenbezogenen Daten ungenau sind, nicht mehr für unsere Geschäftszwecke erforderlich sind oder wenn Sie der Verarbeitung Ihrer personenbezogenen Daten widersprechen, haben Sie auch das Recht, zu verlangen, dass wir die Verarbeitung Ihrer Daten einstellen, während wir Ihren Anspruch untersuchen und/oder überprüfen.
- Ihre personenbezogenen Daten unter bestimmten Umständen löschen oder einschränken zu lassen. Zum Beispiel, wenn Ihre personenbezogenen Daten auf der Grundlage Ihrer Zustimmung verwendet wurden und es keine andere rechtliche Grundlage für die Verwendung gibt, können Sie verlangen, dass Ihre personenbezogenen Daten gelöscht werden, wenn Sie Ihre Zustimmung widerrufen. Ein Widerruf Ihrer Zustimmung wirkt sich nicht auf die Rechtmäßigkeit der Verarbeitung aus, die wir vor Ihrem Widerruf durchgeführt haben, noch wird es die Verarbeitung Ihrer personenbezogenen Daten beeinträchtigen, die auf rechtmäßige Verarbeitungsgründe außerhalb der Zustimmung gestützt ist.

Wenn wir Sie bitten, uns personenbezogene Daten zur Erfüllung einer rechtlichen Anforderung oder zum Abschluss eines Vertrags zur Verfügung zu stellen, werden wir Sie darüber informieren und Ihnen mitteilen, ob die Bereitstellung Ihrer personenbezogenen Daten erforderlich ist und wenn nicht, welche Konsequenzen das Nichtteilen Ihrer personenbezogenen Daten mit uns hat.

Ebenso, wenn wir Ihre personenbezogenen Daten auf der Grundlage unserer oder der berechtigten Interessen eines Dritten sammeln und verwenden und diese Interessen nicht bereits oben beschrieben sind, werden wir Sie darüber informieren, was diese berechtigten Interessen sind.

Wir bemühen uns, geeignete Schutzmaßnahmen zu treffen, um die Privatsphäre und Sicherheit Ihrer personenbezogenen Daten zu schützen und diese nur in einer Weise zu verwenden, die mit Ihrer Beziehung zu uns und den in dieser Datenschutzerklärung beschriebenen Praktiken übereinstimmt.

Um die Zustimmung zu widerrufen oder diese Rechte auszuüben, kontaktieren Sie uns bitte per E-Mail unter [compliance@indicate-data.io](mailto:compliance@indicate-data.io).

Wenn Sie mit unserer Antwort nicht zufrieden sind oder der Meinung sind, dass wir Ihre personenbezogenen Daten rechtswidrig verarbeiten, haben Sie das Recht, eine Beschwerde bei der Aufsichtsbehörde (auch bekannt als Datenschutzbehörde) oder einer anderen zuständigen staatlichen Behörde in Ihrem EWR-Mitgliedstaat einzureichen. Sie haben auch das Recht, eine DSGVO-Beschwerde bei der zuständigen Aufsichtsbehörde einzureichen. Eine Liste der Aufsichtsbehörden finden Sie [hier](https://edpb.europa.eu/about-edpb/board/members_de).

## Sicherheit

Die von uns erhobenen personenbezogenen Daten werden sicher innerhalb unserer Datenbank auf unseren eigenen Servern oder Datenbanken, die von Cloud-Anbietern in gesicherten Umgebungen gehostet werden, gespeichert. Wir verwenden standardmäßig, branchenübliche, kommerziell angemessene Sicherheitspraktiken wie Verschlüsselung, Firewalls und SSL (Secure Socket Layers - 256 Bit) zum Schutz Ihrer Informationen. Eine weitere Beschreibung unserer implementierten Sicherheitsmaßnahmen finden Sie in unserer Sicherheitsrichtlinie.

Wir können jedoch nicht garantieren, dass die Sicherheit von Informationen, die Sie an Indicate Data übermitteln, gewährleistet ist, und Sie tun dies auf eigenes Risiko. Sobald wir Ihre Übermittlung von Informationen erhalten haben, bemüht sich Indicate Data, kommerziell angemessene Anstrengungen zu unternehmen, um die Sicherheit unserer Systeme sicherzustellen. Beachten Sie jedoch, dass dies keine Garantie dafür ist, dass solche Informationen nicht durch Verletzung einer unserer physischen, technischen oder verwaltungstechnischen Sicherheitsvorkehrungen zugegriffen, offengelegt, geändert oder zerstört werden können. Bitte geben Sie Ihr Kontopasswort nicht an eine andere Person weiter. Wir können nicht für einen Datenschutzverstoß oder einen anderen Vorfall verantwortlich gemacht werden, wenn ein Passwort von unbefugten Personen verwendet wird.

Wir bewahren Ihre personenbezogenen Daten in aktiven Dateien oder Systemen so lange auf, wie es erforderlich ist, um die Zwecke zu erfüllen, für die sie gesammelt wurden, um die angeforderten Dienste oder Produkte zu erbringen oder wie es gesetzlich erforderlich sein kann.

## Datenschutz für Kinder

Der Schutz der Privatsphäre von Kindern ist besonders wichtig. Indicate Data und seine Produkte und Dienstleistungen sind für Erwachsene ab 18 Jahren gedacht. Da unsere Inhalte und Dienstleistungen nicht auf Kinder ausgerichtet sind, dürfen Kinder unter 16 Jahren sich nicht auf unserer Website registrieren oder an öffentlichen Posting-Bereichen auf unserer Website teilnehmen. Wenn Sie unter 16 Jahre alt sind, senden Sie uns bitte keine Informationen über sich selbst, einschließlich Ihres Namens, Ihrer Adresse, Telefonnummer oder E-Mail-Adresse. Niemand unter 16 Jahren darf personenbezogene Daten an oder über Indicate Data bereitstellen. Wenn wir feststellen, dass wir personenbezogene Daten von einem Kind unter 16 Jahren ohne elterliche Zustimmung gesammelt haben, wird Indicate Data diese Informationen so schnell wie möglich, jedoch spätestens innerhalb von 30 Tagen, löschen.

## Vertraulichkeit von Nutzerinhalten und Nachrichten

Ihre Nutzerinhalte bleiben in Ihrem Netzwerk privat. Indicate Data überwacht die von Ihnen auf der Website veröffentlichten Nutzerinhalte nicht. Wir verfügen nicht über administrative Tools, um die Nachrichten unserer Benutzer einzusehen, und der Zugriff auf die Datenbank wird unseren Technikern nur fallweise gewährt, um spezifische technische Probleme zu beheben oder wie gesetzlich erforderlich. Die Regeln für die Veröffentlichung von Nutzerinhalten sind in unseren Nutzungsbedingungen festgelegt.

## Meine Wahlmöglichkeiten und Rechte

Sofern in dieser Datenschutzerklärung oder in den Nutzungsbedingungen, soweit anwendbar, nicht anders beschrieben, verwenden wir personenbezogene Daten nur für die oben beschriebenen Zwecke oder wie sonst zum Zeitpunkt unserer Anfrage nach solchen Informationen von Ihnen offengelegt. Sie müssen "opt in" wählen und uns die Erlaubnis geben, Ihre personenbezogenen Daten für andere Zwecke zu verwenden. Sie können Ihre Präferenz jederzeit ändern und sich von bestimmten Marketingkommunikationen von uns abmelden, indem Sie auf den Link "Abmelden" klicken, der in allen solchen Kommunikationen enthalten ist, oder indem Sie uns per E-Mail unter [compliance@indicate-data.io](mailto:compliance@indicate-data.io) kontaktieren.

## Kann ich meine Informationen aktualisieren oder korrigieren?

Sie können anfragen, die Informationen über Sie, die Indicate Data gespeichert hat, einzusehen und zu korrigieren. Kunden haben das Recht, die Einschränkung bestimmter Verwendungen und Offenlegungen personenbezogener Informationen wie folgt anzufordern. Sie können uns kontaktieren, um (1) Ihre personenbezogenen Informationen zu aktualisieren oder zu korrigieren, (2) Ihre Präferenzen in Bezug auf Kommunikation und andere Informationen, die Sie von uns erhalten, zu ändern, oder (3) die über Sie in unseren Systemen gespeicherten personenbezogenen Informationen zu löschen (vorbehaltlich des folgenden Absatzes), indem Sie Ihr Konto kündigen. Solche Aktualisierungen, Korrekturen, Änderungen und Löschungen haben keine Auswirkungen auf andere Informationen, die wir speichern, oder Informationen, die wir Dritten gemäß dieser Datenschutzerklärung vor einer solchen Aktualisierung, Korrektur, Änderung oder Löschung bereitgestellt haben.

Sie können Ihre Kontoinformationen und E-Mail-Präferenzen jederzeit aktualisieren oder korrigieren, indem Sie sich in Ihr Konto einloggen. Um Ihre Privatsphäre und Sicherheit zu schützen, können wir angemessene Schritte unternehmen (wie die Anforderung eines eindeutigen Passworts), um Ihre Identität zu überprüfen, bevor wir Ihnen Zugang zu Ihren Profileinstellungen gewähren oder Korrekturen vornehmen. Sie sind jederzeit für die Geheimhaltung Ihres eindeutigen Passworts und Ihrer Kontoinformationen verantwortlich.

Es ist technologisch nicht möglich, jede einzelne Aufzeichnung der Informationen, die Sie uns zur Verfügung gestellt haben, aus unserem System zu entfernen. Die Notwendigkeit, unsere Systeme zu sichern, um Informationen vor unbeabsichtigtem Verlust zu schützen, bedeutet, dass eine Kopie Ihrer Informationen in einer nicht löschbaren Form vorhanden sein kann, die schwer oder unmöglich für uns zu lokalisieren sein wird. Unverzüglich nach Erhalt Ihrer Anfrage werden alle personenbezogenen Daten, die in Datenbanken gespeichert sind, die wir aktiv nutzen, und anderen leicht durchsuchbaren Medien, so bald und in dem Maße, wie es vernünftigerweise und technisch praktikabel ist, aktualisiert, korrigiert, geändert oder gelöscht.

Sie können natürlich auch darauf verzichten, Ihre personenbezogenen Daten über Indicate Data einzureichen, in welchem Fall Indicate Data möglicherweise nicht in der Lage ist, bestimmte Dienste für Sie zu erbringen.

## Kompromittierung von personenbezogenen Daten

Für den Fall, dass personenbezogene Daten aufgrund eines Sicherheitsverstoßes kompromittiert werden, wird Indicate Data die betroffenen Personen, deren personenbezogene Daten kompromittiert wurden, unverzüglich benachrichtigen, gemäß den in dieser Datenschutzerklärung festgelegten Benachrichtigungsverfahren oder wie anderweitig gesetzlich vorgeschrieben.

Bitte beachten Sie, dass, wenn Sie personenbezogene Daten in öffentlichen Bereichen, auf öffentlichen Schwarzen Brettern oder innerhalb der Website freiwillig offenlegen, diese Informationen von Dritten gesammelt und verbreitet werden könnten und dies unter anderem unerwünschte Anfragen, Nachrichten und Angebote von Dritten zur Folge haben könnte. Bitte haben Sie Verständnis dafür, dass alle Informationen, die in diesen Bereichen offengelegt werden, öffentliche Informationen werden und dieses Verhalten Dritter außerhalb der Kontrolle von Indicate Data liegt.

Bitte beachten Sie auch, dass, wenn Sie personenbezogene Daten in Ihrem Konto freiwillig offenlegen, alle Benutzer, die berechtigt sind, auf solche Bereiche zuzugreifen (Benutzer in Ihrem Konto), Zugang zu diesen personenbezogenen Daten haben werden, und diese Informationen könnten von diesen Benutzern gesammelt und verbreitet werden und könnten unter anderem unerwünschte Anfragen, Nachrichten und Angebote von solchen Benutzern zur Folge haben. Indicate Data rät Ihnen, Vorsicht walten zu lassen, wenn Sie entscheiden, personenbezogene Daten in der Webanwendung oder auf der Website offenzulegen.

## Links zu anderen Websites

Die Website oder Webanwendungen können auch Links zu Websites Dritter enthalten, die von Dritten besessen oder kontrolliert werden. Wir sind nicht verantwortlich für die Datenschutzpraktiken oder den Inhalt solcher Websites Dritter. Bevor Sie auf eine Website zugreifen, wird empfohlen, die anwendbaren Datenschutz- und anderen Richtlinien zu überprüfen.

## Änderungen an der Datenschutzerklärung

Wenn wir diese Datenschutzerklärung ändern, werden wir diese Änderungen auf unserer Website veröffentlichen und die Benutzer gemäß unseren Benachrichtigungen zu dieser Datenschutzerklärung informieren, um Sie über Aktualisierungen und Änderungen bezüglich der Informationen, die wir erfassen, wie wir sie verwenden und unter welchen Umständen wir sie offenlegen können, auf dem Laufenden zu halten. Änderungen an dieser Datenschutzerklärung sind ab dem oben angegebenen Datum wirksam.

Wir werden jedoch keine Änderungen vornehmen, die zu wesentlichen zusätzlichen Verwendungen oder Offenlegungen Ihrer personenbezogenen Daten führen, ohne Sie über solche Änderungen per E-Mail mindestens 14 Tage vor Inkrafttreten dieser Änderungen zu benachrichtigen.

Wenn Sie mit Änderungen an dieser Datenschutzerklärung, die nicht erheblich sind, nicht einverstanden sind, müssen Sie uns sofort kontaktieren und, bis das Problem gelöst ist, die Nutzung der Indicate Data-Website und der von uns bereitgestellten Dienste einstellen. Ihre fortgesetzte Nutzung unserer Dienste und der Indicate Data-Website und Webanwendung nach der Veröffentlichung von nicht wesentlichen Änderungen an dieser Datenschutzerklärung stellt Ihre Zustimmung zu diesen Änderungen dar.

## Fragen zu dieser Datenschutzerklärung

Wenn Sie Fragen oder Kommentare zu dieser Datenschutzerklärung haben oder der Meinung sind, dass wir nicht nach den Bedingungen dieser Datenschutzerklärung oder den geltenden Datenschutzgesetzen handeln, kontaktieren Sie bitte unseren Datenschutzbeauftragten auf eine der folgenden Arten:

Per Post oder Kurier: Indicate Data GmbH Industriestr 27 D-77656 Offenburg Deutschland Per E-Mail: [compliance@indicate-data.io](mailto:compliance@indicate-data.io)

## Benachrichtigungen zu dieser Datenschutzerklärung

Wenn wir unsere Benutzer über diese Datenschutzerklärung benachrichtigen müssen, können wir dies unter Verwendung der Kontaktdaten tun, die mit Ihrem Konto verknüpft sind oder uns anderweitig zur Verfügung gestellt wurden.`

const en = `We are committed to protecting the privacy of our visitors and members. This Privacy Policy explains which personal and other data of yours is collected by Indicate Data on its website (www.indicate-data.io) and our web applications, how the information is used and how you can control the collection, use, correction and/or deletion of your information. We will not pass on or share your information with third parties except as described in this Privacy Policy.

The "Company", "Indicate Data", "Indicate", "us", "we" or "our" refers to Indicate Data GmbH., the owner and operator of the website and web applications, a company registered in Germany.

This Privacy Policy is part of our Terms of Service. BY USING OUR WEBSITE / WEB APPLICATIONS YOU AGREE TO THE TERMS OF THIS PRIVACY POLICY.

Please read this Privacy Policy carefully in order to understand our privacy practices. If you do not agree to these practices, your remedy is not to use the website or web applications.

## What Information We Collect

Information you provide to us - You give us information about yourself, such as your name and your e-mail address, when you register with us. If you correspond with us by e-mail, telephone or through the website, we may store the content of your messages, your e-mail address or telephone number and our replies.

Registration on our website - Persons who access our web applications are asked to register through our website (signing up for a free/paid account). During registration you will be asked to provide some personal information: your name, your company affiliation and your company e-mail address. A unique password for access to your account is also required. (Please do not disclose your password to anyone.)

When you register and set up an account, and when accessing our web applications, you will be asked to confirm your associated profile settings, including information on how you would like to be contacted.

You can also register and create an account via Google. Personal data that you have provided to Google will be made available to us in order to set up your account. If you sign up for an account with us via Google, you agree to this Privacy Policy and our Terms of Service.

We do not collect "sensitive" or "special" categories of personal data.

User content - There may be public areas on the website or in the web applications where users can post messages and take part in public bulletin boards. The rules for posting user content are governed by our Terms of Service.

Information you provide automatically - When you access the website, we automatically collect certain personal and other information, for example about the device you are using and how you interact with the website and the web applications. Information collected may specifically include: (1) the browser types and versions used, (2) the operating system used by the device, (3) the website from which you access our website (known as the referrer), (4) the sub-websites, (5) the date and time (known as the time stamp) of access to the internet page and (6) the internet protocol address (IP address) assigned to your device. We store this information in server log files. We also automatically collect information about your use of the website and web applications, such as web requests, number of clicks, domain names, pages viewed and the time spent on particular pages.

This information is required in order to (1) deliver the content of our website and web applications correctly, (2) optimize the content of our website and web applications, (3) ensure the long-term viability of our information technology systems and of the website technology and (4) provide law enforcement authorities with the information necessary for criminal prosecution in the event of a cyberattack.

In addition, tracking information is collected as you navigate through our website or web applications. In order to meet your needs better, we use "cookies" to store and sometimes track user information. A cookie is a small amount of data that is sent from a web server to your browser and stored on the hard drive of your device. Cookies can be disabled or controlled through the settings in your browser. You can also delete or refuse cookies. Further information can be found in our Cookie Policy.

In addition, log file information is reported automatically by your browser every time you call up a web page. When you register with or view our website or web applications, our servers automatically record certain information that your browser sends every time you visit a website.

Indicate Data also analyzes anonymously collected data and statistics in order to improve your experience and to ensure an optimal level of protection for the personal data we process. The anonymous data from the server log files is stored separately from all personal data provided by users.

Company information - You may provide further information about yourself and your company, such as your role, industry, company size, annual revenue, etc. Such information is stored securely and may be used for benchmarking and other purposes. Indicate Data may collect such information with the help of third-party providers acting as subcontractors in order to ensure a better experience and profiling. You can update or correct this information at any time by logging in to your account.

This personal data is not passed on to third parties. You can unsubscribe from our online newsletter at any time by clicking the "Unsubscribe" link at the end of our e-mails or by contacting us directly at: [compliance@indicate-data.io](mailto:compliance@indicate-data.io).

## Benchmarks

Indicate Data may use your data to calculate and provide benchmarking services. The data collected for participation in the benchmarking ecosystem is anonymized at all times and kept separate from any personal or sensitive information.

In order to ensure anonymity, benchmarks are only calculated and made available if Indicate Data has at least 10 different data sources within the selected cohort.

You can decline to take part in benchmarks at any time by logging in to your account and opting out.

## How We Use Your Information

In general, we use the personal data and other information collected in order to provide you with the best possible experience when using the Indicate Data website and web applications. This information also helps us to understand who uses our products and how we can improve our services, in order to provide our helpdesk service, to contact you for customer service and billing reasons and to facilitate the provision of Indicate Data marketing information about our products and services.

We use the personal data you submit in order to operate, maintain and provide to you the features and functionalities of our products and services. Any personal data or user content that you voluntarily disclose by posting on Indicate Data may become available to the other users in your account. If you remove user content, copies may remain visible in cached and archived pages or where other users have copied or stored your user content.

We will use your registered e-mail address to send you Indicate Data related notices about products or services you have requested, about the status of your account or about other administrative or technical information, including notices required by law. We may also use your e-mail address to send you other messages, such as newsletters, changes to features within Indicate Data or special offers. If you do not wish to receive such e-mail messages, you can unsubscribe at any time in your profile settings or click the "Unsubscribe" link at the end of our e-mails. You can also unsubscribe from receiving our marketing messages by contacting us at [compliance@indicate-data.io](mailto:compliance@indicate-data.io). We do not pass on your e-mail or other personal data to third parties for their marketing activities. We also do not engage in targeted advertising, neither on our website nor on third-party websites.

We use cookies, clear GIFs and information from server log files in order to store information so that you do not have to enter it again during your visit or the next time you visit the website; to provide individual, personalized content and information; to monitor the effectiveness of our service; to monitor aggregate metrics such as the total number of visitors and traffic; to diagnose or fix technical problems reported by our users or engineers that are associated with the IP addresses of a particular web company or ISP; and to help you access your information efficiently after signing in. Further information about our use of cookies can be found in our Cookie Policy.

## How We Share Your Information

We may engage independent contractors, providers and suppliers (collectively "external contractors") in order to provide certain services and products in connection with the Indicate Data website and our services, such as hosting, customer service, credit card processing and fraud screening as well as hosting of mailing lists. In the course of providing products or services to us, these external contractors may have access to information collected via the Indicate Data website or web application, including your personal data. We require that these external contractors (1) protect the privacy of your personal data in accordance with this Privacy Policy and the Terms of Service and (2) do not use or disclose your personal data for purposes other than providing the products or services for which we engaged them, or as required by law.

When you use our website and web applications, you may interact with our business partners. In order to provide you with additional support, your name and your e-mail address may be shared with partners.

As we develop our business further, we may buy or sell assets or business offerings. Customer and visitor information is generally one of the business assets transferred in these types of transactions. We may also transfer such information in the course of corporate divestitures, mergers or dissolutions.

Indicate Data may disclose personal data where this is required by law or by subpoena, or where we believe that such action is necessary in order to comply with the law, to comply with legal proceedings served on us or our affiliated companies, or to conduct investigations, to take precautions or to take measures with regard to suspected or actual illegal activities; to enforce our Terms of Service, to take precautions against liability, to defend ourselves against claims or allegations by third parties, to assist law enforcement authorities or to protect the security or integrity of our website; and to protect or exercise the rights, the property or the personal safety of Indicate Data, our users or others.

In addition, we may carry out statistical analyses of customer usage in order to measure interest in and use of the various parts of the website, and we may share this information in the course of our business, but only in anonymized and/or aggregated form (statistics, etc.), and without personal data.

Indicate Data does not rent or sell your personal data to third parties.

## Processors We Engage

In order to provide our services, we engage carefully selected service providers as processors within the meaning of Art. 28 GDPR. We have concluded a data processing agreement (DPA) with each of these service providers. The processors process personal data exclusively on our instructions and for the purposes stated below. The following overview lists the processors we currently engage:

| Processor | Service and purpose of the processing | Place of processing | Basis of the transfer |
|---|---|---|---|
| Google Ireland Limited, Gordon House, Barrow Street, Dublin 4, D04 E5W5, Ireland | Google Analytics – reach measurement and analysis of the use of our website | EU/EEA; transfer to Google LLC, USA | DPA pursuant to Art. 28 GDPR. Google LLC is certified under the EU-U.S. Data Privacy Framework (among other things for Google Analytics); transfers to the USA therefore take place on the basis of the adequacy decision pursuant to Art. 45 GDPR, supplemented by standard contractual clauses pursuant to Art. 46 GDPR. |
| Microsoft Ireland Operations Limited, One Microsoft Place, South County Business Park, Leopardstown, Dublin 18, D18 P521, Ireland | Microsoft 365 – e-mail, office applications and collaboration, including the handling of customer inquiries | EU/EEA; transfer to Microsoft Corporation, USA | DPA pursuant to Art. 28 GDPR. Microsoft Corporation is certified under the EU-U.S. Data Privacy Framework; transfers to the USA therefore take place on the basis of the adequacy decision pursuant to Art. 45 GDPR, supplemented by standard contractual clauses pursuant to Art. 46 GDPR. |
| Productlane GmbH, Albert-Roßhaupter-Straße 3b, 81369 Munich, Germany | Productlane – support platform for receiving and handling support requests and customer feedback | EU (hosting in Frankfurt am Main); individual sub-processors established in the USA | DPA pursuant to Art. 28 GDPR; for transfers to the USA, standard contractual clauses pursuant to Art. 46 GDPR. |
| Linear Orbit, Inc., 2261 Market Street STE 10632, San Francisco, CA 94114, USA | Linear – issue and task management, among other things for tracking support requests | USA | DPA pursuant to Art. 28 GDPR in conjunction with the EU standard contractual clauses pursuant to Art. 46 GDPR. There is no certification under the EU-U.S. Data Privacy Framework. |
| Slack Technologies Limited, Salesforce Tower, 60 R801, North Dock, Dublin, Ireland | Slack – internal communication and coordination, among other things when handling support requests | EU/EEA; transfer to Slack Technologies, LLC and Salesforce, Inc., USA | DPA pursuant to Art. 28 GDPR. Salesforce, Inc., including Slack Technologies, LLC, is certified under the EU-U.S. Data Privacy Framework; transfers to the USA therefore take place on the basis of the adequacy decision pursuant to Art. 45 GDPR, supplemented by standard contractual clauses pursuant to Art. 46 GDPR. |
| Notion Labs, Inc., 2300 Harrison Street, San Francisco, CA 94110, USA | Notion – internal documentation and knowledge management, among other things for support processes | USA | DPA pursuant to Art. 28 GDPR in conjunction with the EU and UK standard contractual clauses pursuant to Art. 46 GDPR. |

We review our processors regularly and update this overview when we add service providers or cease to engage them. If you have questions about any of the processors named or would like a copy of the safeguards put in place for third-country transfers, please contact [compliance@indicate-data.io](mailto:compliance@indicate-data.io).

## California Privacy Rights

California "Shine The Light" - As provided for in California Civil Code 1798.83, if you are a resident of California you have the right to receive information that (a) names every third-party company to which we disclosed your personal data in the past year; and (b) provides a description of the categories of personal data that were disclosed. To request such information, please send your request by e-mail to [compliance@indicate-data.io](mailto:compliance@indicate-data.io) with "California Privacy Rights Request" in the subject line.

California Consumer Privacy Act - In providing its services to its customers, Indicate Data may be considered a "service provider" within the meaning of the California Consumer Privacy Act ("CCPA"). Indicate Data may also use the services of third parties that may likewise be regarded as "service providers" within the meaning of the CCPA. To the extent requested by our customers, we assist in responding to requests from California consumers regarding their privacy rights under the CCPA. If we receive a request from a California consumer to know or to delete which personal data we collect on behalf of our customer for whom we provide services, we will instruct the consumer to address the request directly to our customer. Where possible, we will provide the consumer with the contact details of the customer. You can contact us at [compliance@indicate-data.io](mailto:compliance@indicate-data.io) for further information about these rights.

We do not sell your personal data to third parties for any purpose.

## Location in Germany

Indicate Data and our website and web applications are located in Germany and are subject to German law. If you are located outside Germany when you visit the website or the web application or communicate with us by post, e-mail or telephone, please note that your personal data may be transferred to, stored and processed in Germany, where our servers are located. All information that you provide to us or that we collect through your use of the websites or web applications is stored, processed and transferred in Germany. Please note that Germany and jurisdictions other than the one in which you are located may not offer the same standard of data protection as is considered adequate in your country. Please also note that your personal data may be available to the German government or its authorities in the course of legal proceedings in Germany.

## Privacy Rights in the European Economic Area and in the United Kingdom

### The Legal Basis for the Use of Personal Data in the EEA and in the United Kingdom

For residents of the European Economic Area (EEA) and of the United Kingdom (UK) we point out that your personal data is transferred to and processed in Germany, which has data protection laws that differ from those in your country and may not be as protective. Germany has not sought or obtained an "adequacy decision" from the European Union pursuant to Article 45 of the General Data Protection Regulation ("GDPR"). Our legal basis for collecting and using your personal data is to do so with your consent; where we require the personal data for the performance of a contract or of a requested service, including the provision of our newsletter; or where the collection and use is in our legitimate interest or that of another party and is not overridden by your data protection interests or fundamental rights and freedoms. In some cases we may also be legally obliged to collect the personal data. Where we have collected your personal data with your consent, you may withdraw your consent at any time.

Our retention of your personal data and of all subsequent communications is based on our legitimate interest in providing you with new and potentially relevant materials based on your geography, role or company. As always, you can choose to decline such future communications.

To the extent that we transfer personal data from the EEA to a territory outside the EEA that has not been deemed adequate by the European Commission (such as Germany), we will ensure that such personal data is protected by appropriate contractual provisions or other approved mechanisms.

### Our Legal Obligation to Disclose Your Information

We will disclose a user's personal data without their prior consent only where we have reason to believe that the disclosure of this information is necessary in order to establish the identity of, to contact, or to take legal action against a person or persons who are suspected of having infringed rights or property of Indicate Data or of others who could be harmed by the user's activities, or persons who could infringe these rights and property (intentionally or otherwise). We may also disclose personal data where we have good reason to believe that this is required by law.

### Rights for Residents of the EEA and of the United Kingdom

In addition, if you are a resident of the EEA or of the United Kingdom, you have the right:

- To find out whether we use your personal data, to access your personal data and to obtain copies of your personal data.
- To withdraw at any time and without penalty any express consent you have given to the processing of your personal data.
- To access your personal data and to correct or amend it if it is inaccurate or incomplete.
- To receive a portable copy of some of your personal data which can be transferred to another provider, where the personal data was processed on the basis of your consent.
- If you believe that your personal data is inaccurate, is no longer necessary for our business purposes, or if you object to the processing of your personal data, you also have the right to request that we stop processing your data while we investigate and/or review your claim.
- To have your personal data deleted or restricted in certain circumstances. For example, if your personal data was used on the basis of your consent and there is no other legal basis for the use, you may request that your personal data be deleted if you withdraw your consent. A withdrawal of your consent does not affect the lawfulness of the processing we carried out prior to your withdrawal, nor will it affect the processing of your personal data that is based on lawful grounds for processing other than consent.

If we ask you to provide us with personal data in order to meet a legal requirement or to conclude a contract, we will inform you of this and tell you whether the provision of your personal data is required and, if not, what the consequences are of not sharing your personal data with us.

Likewise, if we collect and use your personal data on the basis of our legitimate interests or those of a third party and these interests are not already described above, we will inform you what these legitimate interests are.

We endeavor to take appropriate protective measures in order to safeguard the privacy and security of your personal data and to use it only in a manner that is consistent with your relationship with us and with the practices described in this Privacy Policy.

To withdraw consent or to exercise these rights, please contact us by e-mail at [compliance@indicate-data.io](mailto:compliance@indicate-data.io).

If you are not satisfied with our response or believe that we are processing your personal data unlawfully, you have the right to lodge a complaint with the supervisory authority (also known as the data protection authority) or with another competent state authority in your EEA member state. You also have the right to lodge a GDPR complaint with the competent supervisory authority. A list of supervisory authorities can be found [here](https://edpb.europa.eu/about-edpb/board/members_de).

## Security

The personal data we collect is stored securely within our database on our own servers or on databases hosted by cloud providers in secured environments. We use standard, industry-customary, commercially reasonable security practices such as encryption, firewalls and SSL (Secure Socket Layers - 256 bit) to protect your information. A further description of the security measures we have implemented can be found in our Security Policy.

However, we cannot guarantee that the security of information you transmit to Indicate Data is assured, and you do so at your own risk. Once we have received your transmission of information, Indicate Data endeavors to make commercially reasonable efforts to ensure the security of our systems. Please note, however, that this is not a guarantee that such information cannot be accessed, disclosed, altered or destroyed through a breach of any of our physical, technical or administrative safeguards. Please do not share your account password with any other person. We cannot be held responsible for a data protection breach or any other incident where a password is used by unauthorized persons.

We retain your personal data in active files or systems for as long as is necessary to fulfill the purposes for which it was collected, to provide the requested services or products, or as may be required by law.

## Privacy Protection for Children

Protecting the privacy of children is particularly important. Indicate Data and its products and services are intended for adults aged 18 and over. Since our content and services are not directed at children, children under the age of 16 may not register on our website or take part in public posting areas on our website. If you are under 16 years of age, please do not send us any information about yourself, including your name, your address, telephone number or e-mail address. No one under the age of 16 may provide personal data to or through Indicate Data. If we determine that we have collected personal data from a child under the age of 16 without parental consent, Indicate Data will delete this information as quickly as possible, but no later than within 30 days.

## Confidentiality of User Content and Messages

Your user content remains private within your network. Indicate Data does not monitor the user content you post on the website. We do not have administrative tools for viewing our users' messages, and access to the database is granted to our engineers only on a case-by-case basis in order to resolve specific technical problems or as required by law. The rules for posting user content are set out in our Terms of Service.

## My Choices and Rights

Unless otherwise described in this Privacy Policy or in the Terms of Service, where applicable, we use personal data only for the purposes described above or as otherwise disclosed at the time we request such information from you. You must "opt in" and give us permission to use your personal data for other purposes. You can change your preference at any time and unsubscribe from certain marketing communications from us by clicking the "Unsubscribe" link that is contained in all such communications, or by contacting us by e-mail at [compliance@indicate-data.io](mailto:compliance@indicate-data.io).

## Can I Update or Correct My Information?

You can request to view and correct the information about you that Indicate Data has stored. Customers have the right to request the restriction of certain uses and disclosures of personal information as follows. You can contact us in order to (1) update or correct your personal information, (2) change your preferences with regard to communications and other information you receive from us, or (3) delete the personal information stored about you in our systems (subject to the following paragraph) by terminating your account. Such updates, corrections, changes and deletions have no effect on other information that we store, or on information that we provided to third parties in accordance with this Privacy Policy prior to such update, correction, change or deletion.

You can update or correct your account information and e-mail preferences at any time by logging in to your account. In order to protect your privacy and security, we may take reasonable steps (such as requesting a unique password) to verify your identity before granting you access to your profile settings or making corrections. You are responsible at all times for keeping your unique password and your account information confidential.

It is not technologically possible to remove every single record of the information you have provided to us from our system. The need to back up our systems in order to protect information from unintentional loss means that a copy of your information may exist in a non-erasable form which will be difficult or impossible for us to locate. Promptly after receipt of your request, all personal data stored in databases that we actively use, and in other readily searchable media, will be updated, corrected, changed or deleted as soon as and to the extent that this is reasonably and technically practicable.

You can of course also refrain from submitting your personal data through Indicate Data, in which case Indicate Data may not be able to provide certain services to you.

## Compromise of Personal Data

In the event that personal data is compromised as a result of a security breach, Indicate Data will notify the affected persons whose personal data was compromised without undue delay, in accordance with the notification procedures set out in this Privacy Policy or as otherwise required by law.

Please note that if you voluntarily disclose personal data in public areas, on public bulletin boards or within the website, this information could be collected and disseminated by third parties and this could result in, among other things, unsolicited requests, messages and offers from third parties. Please understand that all information disclosed in these areas becomes public information and that this conduct by third parties is beyond the control of Indicate Data.

Please also note that if you voluntarily disclose personal data in your account, all users who are authorized to access such areas (users in your account) will have access to this personal data, and this information could be collected and disseminated by those users and could result in, among other things, unsolicited requests, messages and offers from such users. Indicate Data advises you to exercise caution when deciding to disclose personal data in the web application or on the website.

## Links to Other Websites

The website or web applications may also contain links to third-party websites that are owned or controlled by third parties. We are not responsible for the privacy practices or the content of such third-party websites. Before accessing a website, it is recommended that you review the applicable privacy and other policies.

## Changes to the Privacy Policy

If we change this Privacy Policy, we will publish these changes on our website and inform users in accordance with our notifications regarding this Privacy Policy, in order to keep you up to date on updates and changes concerning the information we collect, how we use it and the circumstances in which we may disclose it. Changes to this Privacy Policy are effective from the date stated above.

However, we will not make any changes that result in materially additional uses or disclosures of your personal data without notifying you of such changes by e-mail at least 14 days before those changes take effect.

If you do not agree with changes to this Privacy Policy that are not material, you must contact us immediately and, until the matter is resolved, cease using the Indicate Data website and the services we provide. Your continued use of our services and of the Indicate Data website and web application after the publication of non-material changes to this Privacy Policy constitutes your acceptance of those changes.

## Questions About This Privacy Policy

If you have questions or comments about this Privacy Policy or believe that we are not acting in accordance with the terms of this Privacy Policy or with applicable data protection laws, please contact our data protection officer in one of the following ways:

By post or courier: Indicate Data GmbH Industriestr 27 D-77656 Offenburg Germany By e-mail: [compliance@indicate-data.io](mailto:compliance@indicate-data.io)

## Notifications Regarding This Privacy Policy

If we need to notify our users about this Privacy Policy, we may do so using the contact details that are linked to your account or were otherwise provided to us.`

export const privacyPolicy = { de, en }
